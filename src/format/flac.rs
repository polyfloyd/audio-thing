    ...
    R: io::Read + io::Seek + SeekExt + 'static,
{
    unsafe {
        ...
        if length > 0 {
            ...
            debug!(
                "stream info: {} channels, {} bits, {} hz, {} samples",
                num_channels, sample_size, sample_rate, length
            );
        } else {
            debug!(
                "stream info: {} channels, {} bits, {} hz, length unknown",
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let (frame, block_consumed) = {
            ...
            let frame = {
                ...
                F::from_fn(|ch| F::Sample::decode(block.data[ch][cs]))
            };
    ...
    R: io::Read + io::Seek + SeekExt,
{
    ...
    fn length(&self) -> u64 {
        let total_samples = unsafe { FLAC__stream_decoder_get_total_samples(self.decoder) };
...
unsafe extern "C" fn write_cb<R>(
    _: *const FLAC__StreamDecoder,
    frame: *const FLAC__Frame,
    buffer: *const *const FLAC__int32,
    client_data: *mut os::raw::c_void,
) -> FLAC__StreamDecoderWriteStatus
where
    R: io::Read + io::Seek,
{
    let data = (client_data as *mut DecoderCallbackData<R>)
        .as_mut()
        .unwrap();
...
unsafe extern "C" fn length_cb<R>(
    _: *const FLAC__StreamDecoder,
...
unsafe extern "C" fn metadata_cb<R>(
    _: *const FLAC__StreamDecoder,
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Error::InitFailed(status) => unsafe {
                let s = FLAC__StreamDecoderInitStatusString
                    .as_ptr()
            ...
            Error::BadState(state) => unsafe {
                let s = FLAC__StreamDecoderStateString
                    .as_ptr()
            ...
            Error::Unimplemented {
                ...
                f,
                "Flac format not implemented: {} channels, {} bits, finite: {}",
                nc, ss, kl
            ),
        }
    }
