...
fn find_bitrate(index: u8, version: MpegVersion, layer: MpegLayer) -> Result<Option<u32>, Error> {
    ...
    use self::MpegLayer::*;
    Ok(Some(match (index, version, layer) {
        ...
        (index, v, l) => return Err(Error::UnknownBitrate{
            index: index,
            layer: l,
            version: v,
        }),
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Error::MissingSync => {
                write!(f, "Frame sync not found")
            },
            Error::UnknownBitrate{ index, layer, version } => {
                write!(f, "Unknown bitrate: index={:4b}, layer={:?}, version={:?}", index, layer, version)
            },
        }
    }
}
